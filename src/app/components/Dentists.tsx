...
export function Dentists() {
  ...
  const filteredDentists = mockDentists.filter(dentist =>
    `${dentist.fname} ${dentist.lname}`.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setShowModal(false);
    setFormData({ fname: '', lname: '', specialization: '' });
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Dentists</h1>
          </div>
          <p className="text-gray-600 ml-13">Manage dentist profiles</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm text-gray-600">Specialization</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredDentists.map((dentist) => (
                <tr key={dentist.dentist_id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">{dentist.dentist_id}</td>
                  <td className="px-6 py-4">{dentist.fname}</td>
                  <td className="px-6 py-4">{dentist.lname}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-emerald-100 transform transition-all">
            <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-teal-50">
              <h2 className="text-2xl bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Add New Dentist</h2>
            </div>
